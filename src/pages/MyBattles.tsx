    ...
    return (
      <div className="container mx-auto max-w-2xl px-4 py-24 text-center">
        ...
        <p className="text-gray-400 mb-8">
          You need to be logged in to view your battle history.
        </p>
        <Button
          size="lg"
          onClick={signIn}
      ...
      {battles.length === 0 ? (
        <Card className="bg-slate-50 dark:bg-slate-900 border-dashed">
          <CardContent className="py-16 text-center">
            ...
            <p className="text-slate-500 mb-6 max-w-sm mx-auto">
              You haven't created any thumbnail battles yet. Start your first
              A/B test to see how your audience votes.
            </p>
            <a href="/thumbnail-battle">
        ...
        <div className="grid gap-4">
          {battles.map((battle) => {
            ...
            const aPercent =
              totalVotes > 0
                ? Math.round(((battle.votesA || 0) / totalVotes) * 100)
                : 0;
            const bPercent =
              totalVotes > 0
                ? Math.round(((battle.votesB || 0) / totalVotes) * 100)
                : 0;
            ...
            return (
              <Card
                key={battle.id}
                className="overflow-hidden hover:border-slate-300 dark:hover:border-slate-700 transition-colors"
              >
                <div className="p-0 sm:p-6 sm:flex sm:items-center sm:gap-6">
                      ...
                      <div className="space-y-3">
                        <div className="relative">
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span className="font-semibold">
                              {battle.votesA} votes ({aPercent}%)
                            </span>
                          </div>
                          <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2">
                            <div
                              className={`h-2 rounded-full ${isAWinner ? "bg-green-500" : "bg-slate-400"}`}
                        ...
                        <div className="relative">
                          <div className="flex justify-between text-xs mb-1">
                            ...
                            <span className="font-semibold">
                              {battle.votesB} votes ({bPercent}%)
                            </span>
                          </div>
                          <div className="w-full bg-slate-100 dark:bg-slate-800 rounded-full h-2">
                            <div
                              className={`h-2 rounded-full ${isBWinner ? "bg-green-500" : "bg-slate-400"}`}
                  ...
                  <div className="flex sm:flex-col gap-2 p-6 sm:p-0 bg-slate-50 dark:bg-slate-900/50 sm:bg-transparent border-t sm:border-t-0 border-slate-100 dark:border-slate-800">
                    <Button
                      variant="outline"
                      className="flex-1 sm:flex-none w-full gap-2"
                      onClick={() => copyLink(battle.id)}
                    >
                      {copiedId === battle.id ? (
